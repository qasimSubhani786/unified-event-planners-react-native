...
const {validate, Highlights} = require("../model/highlights");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const storageRef = ref(storage, `highlights/${req.file.originalname + " " + getCurrentDateTime()}`);
    const metaData = {contentType: req.file.mimetype};
    // upload file in bucket storage
    const snapshot = await uploadBytesResumable(storageRef, req.file.buffer, metaData);
    const downloadUrl = await getDownloadURL(snapshot.ref);
    let event = new Highlights({
...
router.get(
  "/",
  asyncHandler(async (req, res) => {
    let events = await Highlights.find().lean().select({title: 1, image: 1, _id: 1});
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    let events = await Highlights.findById(id ?? 0);
    if (!events) {
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    let result = await Highlights.findByIdAndUpdate(
      id,
      {
        $set: {
          title: req.body.title
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    const id = validateObjectId(req.params.id);
    let result = await Highlights.findByIdAndRemove(id);
    if (result) {
      res.status(200).send(result);
    } else {
